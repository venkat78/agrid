...
namespace tmesh {
  ...
  INT tCONSTRUCT_MANIFOLDS<_Mesh_Type>::AddToZipper(cHALF_EDGE *he, iMANIFOLD index,
						    sMANIFOLD_ZIPPER *entries,INT numEntries)
  {
  ...
  template <typename _Mesh_Type>
  VOID tCONSTRUCT_MANIFOLDS<_Mesh_Type>::WalkTwoManifoldEdge(cHALF_EDGE *facetHe, cFACET *facet,
							     std::vector<BOOL> &facetsToBeFlipped,
							     INT &numBorderEdges, cMANIFOLD *manifold)
  {
    cHALF_EDGE* facetHeOpp = facetHe->Opp();
    printf("WTME 118: f %d; HET %d HEH %d; Opp: T %d H %d ",
    		facet->Index(), facetHe->Tail()->Index(), facetHe->Head()->Index(),
    ...
    if(!facetHeOpp->IsBorder()) {
      ...
      if(nextFacet->ManifoldIndex() < 0) {
        ...
        if((facetShouldFlip && !nextFacetShouldFlip) ||
           (!facetShouldFlip && nextFacetShouldFlip))
  ...
  template <typename _Mesh_Type>
  VOID tCONSTRUCT_MANIFOLDS<_Mesh_Type>::WalkCodirectedEdge(cHALF_EDGE *facetHe, cFACET *facet,
                                                std::vector<BOOL> &facetsToBeFlipped,
                                                cMANIFOLD *manifold)
  {
    cHALF_EDGE* facetHeOpp = (facetHe->NextInRing()
                             ->NextInRing());
    ...
    if(nextFacet->ManifoldIndex() < 0) {
      ...
      if((nextFacetShouldFlip && facetShouldFlip) ||
         (!nextFacetShouldFlip && !facetShouldFlip))
        manifold->Orientation(NON_ORIENTABLE);
    }
  }

  template <typename _Mesh_Type>
  eORIENTATION tCONSTRUCT_MANIFOLDS<_Mesh_Type>::Construct(cFACET *facet, std::vector<BOOL> &facetsToBeFlipped,
					       sTRUE a)
  {
    ...
    while(!m_stack.empty()) {
      ...
      for( ; nextHe != lastHe ; nextHe++ ) {
        ...
        if(!facetHe->IsNonManifold()) {
          if(!facetHe->IsARing()){
        	WalkTwoManifoldEdge(facetHe, facet, facetsToBeFlipped,
                                numBorderEdges, manifold);
           }
          else {
   ...
   */
  template <typename _Mesh_Type>
  eORIENTATION tCONSTRUCT_MANIFOLDS<_Mesh_Type>::Construct(cFACET *facet,
							   std::vector<BOOL> &facetsToBeFlipped,
							   sFALSE a)
  {
    ASSERT(m_stack.empty());
  ...
  VOID tCONSTRUCT_MANIFOLDS<_Mesh_Type>::OrientManifolds(std::vector<BOOL> &facetsToBeFlipped)
  {
    ...
    for( ; currFacet != lastFacet ; currFacet++) {
      if(!currFacet->IsDeleted() && facetsToBeFlipped[currFacet->Index()]
         && !m_mesh.Manifold(currFacet->ManifoldIndex())->IsNonOrientable()) {
  ...
  VOID tCONSTRUCT_MANIFOLDS<_Mesh_Type>::StitchManifoldsAtNmEdge(cHALF_EDGE *nmEdge)
  {
    ...
    do {
      if(!loopingHe->IsBorder())
        numManifolds += AddToZipper(loopingHe, loopingHe->Facet()->ManifoldIndex(),
                                    zipper, numManifolds);
  ...
  template <typename _Mesh_Type>
  INT tCONSTRUCT_MANIFOLDS<_Mesh_Type>::Construct(INT round)
  {
    if(round == 1)
      BuildQueue(true);
    else if(round == 2)
