...
public:
  tCUT_CELL_CLAY_VERTEX() : cBASE(), m_isReflection(false),
			    m_reflection(INVALID_IVERTEX),
			    m_srcIndex(INVALID_IVERTEX),
			    m_color(UNKNOWN)
  {}
  
  tCUT_CELL_CLAY_VERTEX(const cPOINT3 &point) : cBASE(point), m_isReflection(false),
						m_reflection(INVALID_IVERTEX),
						m_srcIndex(INVALID_IVERTEX),
						m_srcMesh(NULL),
						m_color(UNKNOWN)
...
public:
  tCUT_CELL_CLAY_FACET() : cBASE()
  {}

  tCUT_CELL_CLAY_FACET(const cSELF &f) : cBASE(f),
					 m_color(f.m_color)
  {}
...
TYPEDEF_SUBMESH(cCUT_CELL_CLAY, tMESH, sCUT_CELL_CLAY_STORAGE_POLICY, tMESH_OBJECT_FACTORY,
		tCUT_CELL_CLAY_VERTEX, tHALF_EDGE, tCUT_CELL_CLAY_FACET);

struct sCUT_CELL_CLAY_FILTER
{
public:
  sCUT_CELL_CLAY_FILTER(cCUT_CELL_CLAY *clay = NULL) : m_clay(clay)
  {}
