...
public:
	cGEOM_MGR() :
			m_bounds(cPOINT2(0, 0), cPOINT2(2, 2)) {
...
void test_quad_tree() {
	...
	cQUAD_TREE tree(cBOX2(cPOINT2(0.0, 0.0), cPOINT2(2.0, 2.0)), &mgr);
...
void print_manifolds(cSURFACE_MESH &mesh) {
	...
	for (; currManifold != lastManifold; currManifold++) {
		...
		cSURFACE_MESH::cMANIFOLD::facet_iterator currFacet =
				currManifold->FacetsBegin();
		cSURFACE_MESH::cMANIFOLD::facet_iterator lastFacet =
				currManifold->FacetsEnd();
...
VOID test_off_import(char *filename) {
	...
	ExportToOff<cCUT_CELL, sFACET_FILTER>("cell_after_import.off", m_cell,
			filter);
}
...
int export_to_openfoam(cVOLUMETRIC_GRID &grid, cSURFACE_MESH *bfacesMesh,
	...
	std::string cmd5 = "cp " + fileName + "  " + filePath
			+ "/constant/triSurface/";
	system(cmd5.c_str());
	...
	std::string cmd7 = "cp -R " + filePath + "/constant/polyMesh " + filePath
			+ "/0";
	system(cmd7.c_str());
...
cBOB generate_bob(cBOX3 &simVol, cBOX3 &meshBoundingBox, INT numCells[3]) {
	...
	DO_COORDS(coord)
	{
		REAL u = (meshBoundingBox.MinCoord(coord) - simVol.MinCoord(coord))
				/ (simVol.MaxCoord(coord) - simVol.MinCoord(coord))
				* numCells[coord];
...
BOOL agrid_test(cBOX3 &simVol, INT numCells[3],
		std::vector<cSURFACE_MESH*> &meshes, cCELL_OFF_WRITER &cellWriter,
...
//			numGrayGridCells[1], numGrayGridCells[2]);

	cCUT_CELLS_GENERATOR generator(simVol, numCells);
//	cCUT_CELLS_GENERATOR generator(grayGridVol, numGrayGridCells);
	...
	tmesh::ExportToOff<cVOLUMETRIC_GRID, sVOLUMETRIC_GRID_BOUNDARY_FILTER>(
			"bfacesMesh.off", grid, filter);
...
//  printf("RAND_MAX = %ld\n", RAND_MAX);
	...
	iFACET f0 = (INT) (r0 * c);
//  printf("fo = %d\n", f0);
	cSURFACE_MESH::cFACET::half_edge_circulator currHe =
			mesh.Facet(f0)->HalfEdgesBegin();
	cSURFACE_MESH::cFACET::half_edge_circulator lastHe =
			mesh.Facet(f0)->HalfEdgesEnd();
...
//	printf("index %d; f = %d\n", index, f);
		...
		if (!mesh.Facet(f)->IsDeleted()) {
			...
			cSURFACE_MESH::cFACET::half_edge_circulator currHe =
					mesh.Facet(f)->HalfEdgesBegin();
			cSURFACE_MESH::cFACET::half_edge_circulator lastHe =
					mesh.Facet(f)->HalfEdgesEnd();
...
//      }
	...
	do {
		...
		for (INT i = 1; i < mesh.NumManifolds(); i++) {
			cSURFACE_MESH::cMANIFOLD *manifold0 = mesh.Manifold(
					manifoldIndex[i]);
			cSURFACE_MESH::cMANIFOLD *manifold1 = mesh.Manifold(
					manifoldIndex[i - 1]);
	...
	fscanf(datafile, "%lf %lf %lf %lf %lf %lf %d %d %d", boxCoords,
			boxCoords + 1, boxCoords + 2, boxCoords + 3, boxCoords + 4,
			boxCoords + 5, nCells, nCells + 1, nCells + 2);
...
//  printf("generating Grid for Single Manifolds...\n");
	...
	for (INT i = i0; i < i0 + 1; i++) {
		...
		for (INT j = jmin; j < jmin + 1; j++) {
			...
			std::vector<cSURFACE_MESH*> meshes;
			meshes.push_back(&subMesh);
			if (!agrid_test(simVol, nCells, meshes, cellWriter, file_name,
					file_path)) {
				FILE *problems = fopen("problems.txt", "ab+");
