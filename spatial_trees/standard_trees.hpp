...
namespace spatial_trees {
  ...
  public:
    tOCTREE(const cBOX &box = cBOX(), GEOM_MANAGER *mgr = NULL,
	    INT height = 8, INT leafElementsLim = 50) : cBASE(box, mgr, height, leafElementsLim)
    {}
  };
  ...
  public:
    tQUAD_TREE(const cBOX &box = cBOX(), GEOM_MANAGER *mgr = NULL,
	       INT height = 8, INT leafElementsLim = 50) : cBASE(box, mgr, height, leafElementsLim)
    {}
  };
