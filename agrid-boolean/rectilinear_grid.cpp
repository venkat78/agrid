...
cRECTILINEAR_GRID::cRECTILINEAR_GRID(const cBOX3 &simVol, INT numCells[3]) {
	m_simVol = simVol;
...
cRECTILINEAR_GRID::cRECTILINEAR_GRID(const cBOX3 &simVol, INT numCells) {
	INT numGridCells[3];
	for (INT i = 0; i < 3; i++)
...
INT cRECTILINEAR_GRID::ModifiedCellIndex(const cPOINT3 &point, cVECTOR3 normal,
...
VOID cRECTILINEAR_GRID::Walk(BOOL markBlackCells) {
	...
	AddToFront(WHITE, iCELL_INDEX(0, 0, 0), front, alreadyVisited);
	...
	do {
		...
		while (i < oldFrontSize) {
			...
			cGRID_CELL_COLORS::iterator cellColorItr = m_cellColors.find(
					cellIndex);
			eCELL_COLOR cellColor =
					cellColorItr != m_cellColors.end() ?
							cellColorItr->second : UNKNOWN;

			//X direction neighbors.
			AddToFront(cellColor,
					iCELL_INDEX(cellIndex.x - 1, cellIndex.y, cellIndex.z),
			...
			AddToFront(cellColor,
					iCELL_INDEX(cellIndex.x, cellIndex.y - 1, cellIndex.z),
					front, alreadyVisited);
			...
			AddToFront(cellColor,
					iCELL_INDEX(cellIndex.x, cellIndex.y, cellIndex.z - 1),
					front, alreadyVisited);
