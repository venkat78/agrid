...
namespace agrid_boolean {
  ...
  public:
    cMANIFOLD_RECORD() : m_mesh(NULL), m_manifold(NULL)
    {}

    cMANIFOLD_RECORD(cSURFACE_MESH::cMANIFOLD *manifold,
		     cSURFACE_MESH *mesh,
		     cBOX3 bbox, INT numCells[3]) : m_grid(bbox, numCells),
						    m_mesh(mesh),
						    m_manifold(manifold)
    {}

    cSURFACE_MESH* Mesh() { return m_mesh; }
    cSURFACE_MESH::cMANIFOLD* Manifold() { return m_manifold; }
    cRECTILINEAR_GRID* Grid() { return &m_grid; }
