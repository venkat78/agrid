...
namespace io
{
	...
	BOOL ExportModelToOff(char *filename, _MODEL &model) {
		typedef typename _MODEL::cMESH cMESH;
		FILE *fp = fopen(filename, "wb+");
		...
		cMESH mesh;
		typename _MODEL::mesh_iterator currMesh = model.MeshesBegin();
		typename _MODEL::mesh_iterator lastMesh = model.MeshesEnd();

		std::map<iFACET, sCOLOR> facetColors;
		for( ; currMesh != lastMesh ; currMesh++) {
			sCOLOR color = GetColor(currMesh->GetIndex());
			INT numFacetsOld = mesh.NumFacets();
			Append<cMESH, cMESH>(currMesh.operator*(), mesh);
			INT numFacetsNew = mesh.NumFacets();

			for(INT i = numFacetsOld ; i < numFacetsNew ; i++)
				facetColors[i] = color;
		}
